...
class Attendance:
    ...
    def __init__(self,root):
        ...
        #title section
        title_lb1 = Label(bg_img,text="Attendance Management",font=("verdana",30,"bold"),bg="white",fg="navyblue")
        title_lb1.place(relx=0,rely=0,relwidth=1,height=40)

        #========================Section Creating==================================

        # Creating Frame 
        main_frame = Frame(bg_img,bd=2,bg="white") #bd mean border 
        main_frame.place(relx=0,rely=0.06,relwidth=1,relheight=0.75)
        ...
        left_frame.columnconfigure((0,1,2,3), weight=1)
        ...
        table_frame.place(relx=0.02,rely=0.22,relwidth=0.96,relheight=0.55)
        ...
        #Improt button
        save_btn=Button(btn_frame,command=self.importCsv,text="Import CSV",width=12,font=("verdana",12,"bold"),fg="black",bg="navyblue")
        save_btn.grid(row=0,column=0,padx=6,pady=10,sticky=W)

        #Exprot button
        update_btn=Button(btn_frame,command=self.exportCsv,text="Export CSV",width=12,font=("verdana",12,"bold"),fg="black",bg="navyblue")
        update_btn.grid(row=0,column=1,padx=6,pady=8,sticky=W)

        #Update button
        del_btn=Button(btn_frame,command=self.action,text="Edit",width=12,font=("verdana",12,"bold"),fg="black",bg="navyblue")
        del_btn.grid(row=0,column=2,padx=6,pady=10,sticky=W)

        #reset button
        reset_btn=Button(btn_frame,command=self.reset_data,text="Reset",width=12,font=("verdana",12,"bold"),fg="black",bg="navyblue")
        ...
        table_frame.place(relx=0.02,rely=0.08,relwidth=0.96,relheight=0.85)
    ...
    #Update button
        del_btn=Button(right_frame,command=self.update_data,text="Edit",width=12,font=("verdana",12,"bold"),fg="black",bg="navyblue")
        del_btn.grid(row=0,column=1,padx=6,pady=10,sticky=W)
    #Update button
        del_btn=Button(right_frame,command=self.delete_data,text="Delete",width=12,font=("verdana",12,"bold"),fg="black",bg="navyblue")
        del_btn.grid(row=0,column=2,padx=6,pady=10,sticky=W)
    ...
    def update_data(self):
        if self.var_id.get()=="" or self.var_roll.get()=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                Update=messagebox.askyesno("Update","Do you want to update this student's attendance record?",parent=self.root)
                if Update > 0:
    ...
    def exportCsv(self):
        try:
            ...
            with open(fln,mode="w",newline="") as myfile:
                ...
                messagebox.showinfo("Successfuly","Data exported successfully!")
        except Exception as es:
                messagebox.showerror("Error",f"Due to: {str(es)}",parent=self.root)    
